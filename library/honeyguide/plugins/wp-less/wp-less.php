...
if ( ! class_exists( 'wp_less' ) ) {
	...
	class wp_less {
		...
		public function admin_init() {
			...
			$src = $template_directory . '/assets/less/theme/dynamic.less';
			$handle = 'avada-dynamic';

			$this->parse_stylesheet( $src, $handle );
		}
		...
		public function purge_cache( $url ) {
			...
			// Build a varniship
			if( fusion_get_theme_option( 'cache_server_ip' ) ) {
				$varniship = fusion_get_theme_option( 'cache_server_ip' );
